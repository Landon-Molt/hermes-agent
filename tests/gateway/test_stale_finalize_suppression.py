...
class _IgnoreAuthoritativeFinalConsumer(GatewayStreamConsumer):
    """Leave the confirmed prefix stale so the runner must reconcile it."""
...
async def test_stale_finalize_does_not_suppress_complete_response(
    ...
    if result.get("already_sent"):
        ...
        # reconciliation edit/send, not merely by duplicate-suppression state.
...
async def test_unconfirmed_stale_reconcile_falls_through_to_normal_send(
    ...
    monkeypatch.setattr(
        ...
        _IgnoreAuthoritativeFinalConsumer,
    )
